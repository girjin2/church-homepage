...
export default function Admin(){
 ...
 const [tab,setTab]=useState<"dashboard"|"settings"|"sermons"|"notices"|"album"|"bulletins">("dashboard");
 ...
 async function load(){if(!sb)return;const [a,b,c,d,e]=await Promise.all([
   ...
   sb.from("notices").select("*").order("published_at",{ascending:false}).order("created_at",{ascending:false}),
 ...
 async function addNotice(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!sb)return;setBusy(true);const form=e.currentTarget;const f=new FormData(form);const row={title:f.get("title"),body:f.get("body"),published_at:f.get("published_at")};const {error}=await sb.from("notices").insert(row);setMsg(error?error.message:"교회소식을 등록했습니다.");if(!error){form.reset();await load();}setBusy(false);}
 ...
 const nav=[["dashboard","관리 현황"],["settings","교회 기본정보"],["sermons",`설교 ${sermons.length}`],["notices",`교회소식 ${notices.length}`],["album",`교회앨범 ${photos.length}`],["bulletins",`주보 ${bulletins.length}`]] as const;
 return <div className="admin-shell">
   ...
   {tab==="notices"&&<><section className="card"><h2>교회소식 등록</h2><p className="small">교회 전체에 알릴 새로운 소식을 등록합니다. 같은 날짜라면 나중에 등록한 소식이 위에 표시됩니다.</p><form className="form" onSubmit={addNotice}><input name="title" placeholder="소식 제목" required/><textarea name="body" placeholder="소식 내용" required/><input name="published_at" type="date" defaultValue={today()} required/><button className="btn" disabled={busy}>교회소식 등록</button></form></section><div className="spacer"/><DataSection title="최근 교회소식" rows={recentNotices} dateKey="published_at" onDelete={(x:any)=>del("notices",x.id)}/>{pastNotices.length>0&&<><div className="spacer"/><section className="card"><div className="section-row"><div><h2>지난 교회소식</h2><p className="small">30일이 지난 교회소식입니다.</p></div><button className="btn danger" disabled={busy} onClick={clearPastNotices}>모두 삭제</button></div><DataRows rows={pastNotices} dateKey="published_at" onDelete={(x:any)=>del("notices",x.id)}/></section></>}</>}

   {tab==="album"&&<><section className="card"><h2>교회앨범 사진 등록</h2><p className="small">교회앨범은 승인된 회원만 볼 수 있습니다. 휴대폰 사진을 선택하면 최대 1600px, WebP로 자동 압축합니다. 사진 폴더가 500MB를 넘으면 오래된 사진부터 삭제해 약 450MB로 정리합니다.</p><form className="form" onSubmit={addPhoto}><input name="photo_date" type="date" defaultValue={today()} required/><input name="image" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" required/><button className="btn" disabled={busy}>앨범 사진 등록</button></form></section><div className="spacer"/><section className="card"><h2>등록된 앨범 사진</h2>{photos.length?<div className="list">{photos.map((x:any)=><div className="list-row admin-row" key={x.name}><div><strong>{String(x.name).split("__")[0]}</strong><div className="meta">{Math.round(Number(x.metadata?.size||0)/1024)}KB</div></div><button className="btn danger compact" onClick={()=>delPhoto(x.name)}>삭제</button></div>)}</div>:<div className="empty-state">등록된 사진이 없습니다.</div>}</section></>}
